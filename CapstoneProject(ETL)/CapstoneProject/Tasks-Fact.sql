...
FROM TPCDS.INTERMEDIATE.DAILY_AGGREGATED_SALES_TMP; 
  END
  $$;
--create Task for creating daily aggregated sales incrementally
...
--deletes all rows of a table daily_aggregated_sales
truncate table tpcds.intermediate.daily_aggregated_sales;


-------------------------------------weekly sales inventory fact--------------------------------------------------------
...
select * from TPCDS.ANALYTICS.WEEKLY_SALES_INVENTORY_TMP;
...
select * from TPCDS.ANALYTICS.WEEKLY_SALES_INVENTORY;
  ...
  END
  $$;
--create Task for creating weekly aggregated sales incrementally 
...
--deletes all rows of a table weekly_sales_inventory
